...
export const Authorize = (roles) => {
  return async (req, res, next) => {
    try {
      ...
      if (!authToken) {
        return res.status(401).send({ code: 401, message: 'Authorization token is missing' });
      }

      const tokenParts = authToken.split(' ');
      if (tokenParts.length !== 2 || !(tokenParts[0] === 'Bearer' && tokenParts[1])) {
        return res.status(401).send({ code: 401, message: 'Authorization token format is invalid' });
      }
